    ...
    <div
      ...
      <button
        ...
        <div className="flex items-center gap-8 md:gap-20">
          <span className="font-mono text-xs opacity-20 group-hover:opacity-100 transition-opacity tracking-widest">
            [{num}]
          </span>
          <h2
            className={`text-4xl md:text-8xl font-bold tracking-tighter transition-all duration-700 ${isOpen ? "text-black" : "text-black/60 group-hover:text-black"}`}>
            {service.name}
          </h2>
        </div>
        <div
          className={`relative w-14 h-14 md:w-20 md:h-20 rounded-full flex items-center justify-center transition-all duration-500 border ${isOpen ? "bg-black border-black rotate-180" : "bg-white border-black/10 group-hover:border-black"}`}>
          <Plus
...
function PackageCard({ pkg, index }: { pkg: ServicePackage; index: number }) {
  ...
  return (
    <motion.div
      ...
      <div className="mb-12">
        <h4
          className={`text-xs font-black uppercase tracking-[0.2em] mb-4 opacity-40`}>
          {pkg.name}
        </h4>
      ...
      <div className="space-y-6 mb-16 flex-grow">
        {pkg.features.slice(0, 6).map((f, i) => (
          <div key={i} className="flex items-start gap-4 group/item">
            <div
              className={`mt-1 flex-shrink-0 w-2 h-2 rounded-full ${isHighlight ? "bg-yellow-400" : "bg-black"}`}
            />
            <span
              className={`text-sm font-medium ${isHighlight ? "text-zinc-400" : "text-zinc-600"}`}>
      ...
      <button
        className={`w-full py-6 rounded-2xl font-bold tracking-tight transition-all duration-300 flex items-center justify-center gap-3 ${
          isHighlight
...
function ComparisonTable({ packages }: { packages: ServicePackage[] }) {
  const featureNames = Array.from(
    new Set(
      packages.flatMap((pkg) => pkg.features.map((f) => f.name)),
    ),
  );
  ...
  return (
    <div className="w-full overflow-x-auto">
      <table className="w-full border-collapse">
        <thead>
          <tr>
            <th className="py-8 pr-10 text-left text-[10px] font-black uppercase tracking-[0.3em] opacity-30 border-b border-black/5">
              Deliverable
            </th>
            {packages.map((pkg) => (
              <th
                key={pkg.id}
                className="py-8 px-6 text-center text-xl font-bold border-b border-black/5">
                {pkg.name}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {featureNames.map((name, idx) => (
            <tr key={idx} className="group hover:bg-white/50 transition-colors">
              <td className="py-7 pr-10 border-b border-black/5 text-sm font-bold text-zinc-500 group-hover:text-black transition-colors">
                {name}
              </td>
              {packages.map((pkg) => {
                ...
                return (
                  <td
                    key={pkg.id}
                    className="py-7 px-6 border-b border-black/5 text-center">
                    {f?.value === true ? (
                      <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white shadow-sm text-green-600">
                        <Check size={20} strokeWidth={3} />
                      </div>
                    ) : f?.value === false || !f ? (
                      <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-black/5 opacity-20">
                        <X size={20} />
                      </div>
                    ) : (
